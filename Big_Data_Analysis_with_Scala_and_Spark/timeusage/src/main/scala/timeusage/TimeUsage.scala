...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsColumns = columnNames.filter(s => s.startsWith("t01") || s.startsWith("t11") ||
      s.startsWith("t03") || s.startsWith("t1801") || s.startsWith("t1803")).map(col)

    val workingActivitiesColumns = columnNames.filter(s => s.startsWith("t05") || s.startsWith("t1805")).map(col)

    val otherActivitiesColumns = columnNames.filter(
      s=> s.startsWith("t02") || s.startsWith("t04") || s.startsWith("t06") || s.startsWith("t07") ||
        s.startsWith("t08") || s.startsWith("t09") || s.startsWith("t10") || s.startsWith("t12") ||
        s.startsWith("t13") ||s.startsWith("t14") || s.startsWith("t15") ||s.startsWith("t16") ||
        (s.startsWith("t18") && !s.startsWith("t1801") && !s.startsWith("t1805") && !s.startsWith("t1803"))
    ).map(col)

    (
      primaryNeedsColumns,
      workingActivitiesColumns,
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy("working","sex","age")
      .agg(round(avg(summed("primaryNeeds")),1),
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "select working, sex, age, round(avg(primaryNeeds),1)," +
      "round(avg(work),1)," +
      "round(avg(other),1) from "+viewName+
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(k => (k.working, k.sex,k.age))
