...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val occ = for (lang <- langs) yield (lang, occurrencesOfLang(lang, rdd))
    occ.sortBy(-_._2)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(
      article => langs.filter(article.mentionsLanguage).map((_, article))
    ).groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    val res = index.map(pair => (pair._1, pair._2.size)).sortBy(_._2, ascending = false)
    res.collect().toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(
      (article: WikipediaArticle) => langs.map(lang => (lang, if (article.mentionsLanguage(lang)) 1 else 0))
