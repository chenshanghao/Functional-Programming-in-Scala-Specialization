...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  override def descendingByRetweet: TweetList = new Cons(mostRetweeted, this.remove(mostRetweeted).descendingByRetweet)
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(tweet => google.exists(w => tweet.text.contains(w)))
  lazy val appleTweets: TweetSet = allTweets.filter(tweet => apple.exists(w => tweet.text.contains(w)))
