...
trait Huffman extends HuffmanInterface {
  ...
  */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(x => x).map(t => (t._1, t._2.length)).toList
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortBy(x => (x._2, x._1)).map(leaf => Leaf(leaf._1,leaf._2 ))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x1 :: x2 :: xs =>(this.makeCodeTree(x1, x2) :: xs).sortBy(weight)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def auxDecode(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
      case Leaf(c, _) if (bits.isEmpty) => List(c)
      case Leaf(c, _) => c :: auxDecode(tree, bits)
      case Fork(left, _, _, _) if bits.head == 0 => auxDecode(left, bits.tail)
      case Fork(_, right, _, _) => auxDecode(right, bits.tail)
    }
    auxDecode(tree, bits)
  }
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table(table.indexWhere(x => x._1 == char))._2
